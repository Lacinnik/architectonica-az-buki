...
export default function AzBukiModule() {
  ...
  const [tx, setTx] = useState(null); // текущая передача (из 7)
  ...
  const reset = () => { setAz(null); setBuka(null); setTx(null); setNote(""); setInduction(""); setInversion(""); };
  ...
  const saveCombo = () => {
    ...
    const item = { id: uuid(), date: `${todayKey()} ${ts().split(",")[1]?.trim() || ""}`, az, buka, tx, note: note.trim(), induction: induction.trim(), inversion: inversion.trim() };
    setDiary([item, ...diary]);
    reset();
  };

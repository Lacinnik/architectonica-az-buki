...
// Прототип v0.5: 49 Аз + 24 Буки + 7 передач + Индукция/Инверсия + самотесты
// Основа: последняя рабочая версия (v0.4) — расширена без ломающих изменений
// ---------------------------------------------
...
// 24 Буки Перехода — компактные карточки + разворот (данные из «Буки ПЕРЕХОДА.txt»)
const BUKI = [
...
// Локальное хранилище (сохраняем совместимость с v0.4)
...
export default function AzBukiModule() {
  // ↓↓↓ ИСПРАВЛЕНО: убран артефакт "$1" и восстановлены пропущенные состояния ↓↓↓
  ...
  const runSelfTests = () => {
    ...
    // Доп. тесты (добавлены, не меняя существующие)
  ...
  const ResonanceCard = () => (
    <motion.div layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
      ...
      <div className="mt-5 rounded-2xl p-4 bg-black/20 border border-yellow-200/20">
        <div className="mb-2 flex items-center justify_between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={pickRandomTx} className="text-xs px-3 py-1 rounded-lg border border-yellow-300/40 text-yellow-200 hover:bg-yellow-300/10">Случайная передача</button>
            <button onClick={runSelfTests} className="text-xs px-3 py-1 rounded-lg border border-emerald-400/40 text-emerald-200 hover:bg-emerald-300/10">Проверка системы</button>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {TRANSMISSIONS.map((t) => (
            <button key={t.id} onClick={() => setTx(t)}
              className={cls(
      ...
      <div className="mt-5 rounded-2xl p-4 bg-black/15 border border-yellow-200/20">
        ...
        <div className="mt-3 grid md:grid-cols-2 gap-3">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {GEOMETRIES.map(g => (
                <button key={g.id} onClick={()=>setGeometry(g.id)} className={`px-3 py-1.5 rounded-lg border text-xs ${geometry===g.id? 'border-yellow-300 bg-yellow-300/20' : 'border-yellow-200/20 text-yellow-100/90'}`}>{g.label}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {POINT_TYPES.map(p => (
                <button key={p.id} onClick={()=>setPointType(p.id)} title={p.hint} className={`px-3 py-1.5 rounded-lg border text-xs ${pointType===p.id? 'border-yellow-300 bg-yellow-300/20' : 'border-yellow-200/20 text-yellow-100/90'}`}>{p.id}</button>
              ))}
            </div>
          </div>
        </div>
        <div className="mt-3 flex items-center gap-2">
          <button onClick={() => setShowTxSuggest(v=>!v)} className="text-xs px-3 py-1.5 rounded-lg border border-yellow-300/40 text-yellow-200 hover:bg-yellow-300/10">
        ...
        {showTxSuggest && (
          <div className="mt-2 flex flex-wrap gap-2">
            {(POINT_TO_TX[pointType]||[]).map(id => {
              ...
              if(!t) return null;
              return (
                <button key={id} onClick={()=>{ setTx(t); setShowTxSuggest(false); }}
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <button onClick={saveCombo} className="px-4 py-2 rounded-xl bg-yellow-300 text-black font-medium shadow hover:shadow-lg active:translate-y-px">Сохранить в дневник</button>
        <button onClick={reset} className="px-4 py-2 rounded-xl border border-yellow-300/40 text-yellow-200 hover:bg-yellow-300/10">Сбросить выбор</button>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center justify-between gap-6 flex-wrap">
            ...
            <div className="flex gap-2">
              <button onClick={pickRandomAz} className="px-4 py-2 rounded-xl bg-yellow-300 text-black font-medium shadow hover:shadow-lg">Случайный Аз</button>
              <button onClick={pickRandomBuka} className="px-4 py-2 rounded-xl border border-yellow-300/40 text-yellow-200 hover:bg-yellow-300/10">Случайная Бука</button>
              <button onClick={pickRandomTx} className="px-4 py-2 rounded-xl border border-yellow-300/40 text-yellow-200 hover:bg-yellow-300/10">Случайная передача</button>
            </div>
          </div>
          ...
          <div className="mt-6 grid md:grid-cols-2 gap-6">
            {/* Аз (49) */}
            <motion.div layout className="rounded-2xl p-4 bg-black/20 border border-yellow-200/20">
              <div className="mb-3 text-yellow-200/90 font-medium">Выбор Аз (49)</div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {AZ.map((item, idx) => (
                  <button key={item.id} onClick={() => setAz(item)}
                    className={cls(
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-2">
                {BUKI.map((item, idx) => (
                  <div key={item.id} className={cls("rounded-xl border p-3", buka?.id === item.id ? "border-yellow-300 bg-yellow-300/10" : "border-yellow-200/20")}> 
                    <button onClick={() => setBuka(item)} className="w-full text-left">
                    ...
                    <button onClick={() => setOpenBukaId(openBukaId === item.id ? null : item.id)} className="mt-2 text-xs text-yellow-200/80 hover:underline">
          ...
          {/* Резонансная карта */}
          <div className="mt-6">
            <AnimatePresence>{az && buka ? <ResonanceCard /> : null}</AnimatePresence>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        ...
        {diary.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {diary.map((d) => (
              <motion.div key={d.id} layout className="rounded-2xl p-4 bg-black/30 border border-yellow-200/20" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                <div className="flex items-center justify-between gap-3">
                  <div className="text-sm text-yellow-200/70">{d.date}</div>
                  <button onClick={() => removeItem(d.id)} className="text-xs px-2 py-1 rounded-lg border border-yellow-300/30 text-yellow-200 hover:bg-yellow-300/10">Удалить</button>
                </div>
        ...
        {diary.length > 0 && (
          <div className="mt-4 flex gap-3">
            <button onClick={clearAll} className="px-4 py-2 rounded-xl border border-red-400/40 text-red-300 hover:bg-red-400/10">Очистить дневник</button>
          </div>
        )}
      </section>
